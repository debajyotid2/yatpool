/* Single threaded image blurring
...
void generate_worms(Image* img) {
    for (int i = 0; i < NUM_WORMS; ++i) {
        ...
        double nx = (double)rand() / RAND_MAX * 1000.0;
        double ny = (double)rand() / RAND_MAX * 1000.0;
...
void apply_blur(const Image* src, Image* dest) {
...
int main(int argc, char **argv) {
    ...
    printf("Applying a %dx%d box blur...\n", BLUR_KERNEL_SIZE,
           BLUR_KERNEL_SIZE);
    apply_blur(src_image, dest_image);
